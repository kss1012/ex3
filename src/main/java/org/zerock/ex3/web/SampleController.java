...
public class SampleController {
    ...
    @GetMapping({"/ex2", "/ex22", "/ex223", "/ex224", "/ex225",
            "/ex226", "/ex227", "/ex228","/ex229","/exLink","/exDate", "/exDate2"})
    public void exModel(Model model) {
        ...
        for (int i = 1; i <= 20; i++) {
            SampleDTO dto = SampleDTO.builder()
                    ...
                    .build();
            list2.add(dto);  // 위에 코드와 아래 코드는 동일함 위에 코드가 속도는 더 빠름

        }
        //컬렉션 스트림을 이용하여 1~20 까지 정수를 발생합니다. 발생한 정수를 Long 타입으로
